...
;; $Id: mac-key-mode.el 227 2007-11-17 04:16:31Z zenitani $
;; Keywords: tools, mac
;; Created: 2004-12-27
;; Compatibility: Mac OS X 10.4 (Carbon Emacs)
...
(defcustom mac-key-mode-modify-menu-bar t
  "If non-nil, `mac-key-mode' adds several menu items to the File menu
...
(defvar mac-key-invalid-file-regexp ":") ; invalid file + remote file
...
When Mac Key mode is enabled, mac-style key bindings are provided."
  ...
  (if mac-key-mode
      (progn
        ...
        (when mac-key-mode-modify-menu-bar
          (if (boundp 'mac-key-mode-internal) (setq mac-key-mode-internal t))
    ...
    (progn
      ...
      (when mac-key-mode-modify-menu-bar
        (if (boundp 'mac-key-mode-internal) (setq mac-key-mode-internal nil))
...
(defun mac-key-show-in-finder (&optional path)
  ...
  (let ((item (or path
    ...
    (cond
     ((not (stringp item)))
     ((string-match mac-key-invalid-file-regexp item)
  ...
  (let ((editable (not buffer-read-only))
    ...
    (popup-menu
     ...
     ["Look Up in Dictionary"
      ;; dict protocol is unstable, at least on Tiger
;;    (browse-url
;;     (concat "dict:///"
;;             (url-hexify-string (buffer-substring-no-properties beg end))))
;;    (do-applescript
;;     (concat "tell application \"Dictionary\"
;;     activate
;;     open location \"dict:///" (buffer-substring-no-properties beg end) "\"
;; end tell"))
      (progn
        ;; copy selected word to the clipboard
        (clipboard-kill-ring-save beg end)
        ;; ref. http://daringfireball.net/2005/05/dictionary_bbedit
        (do-applescript "tell application \"Dictionary\" to activate
...
--        keystroke return
      ...
      :active t]
     ["--" nil]
     ["Cut"   (clipboard-kill-region beg end) :active editable]
     ["Copy"  (clipboard-kill-ring-save beg end)]
